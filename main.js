...
const PARAM_DEFS = [
  ...
  },
  {
    ...
    type: 'color',
    gen_default: () => 'rgb(18, 52, 82)',
  },
  {
    key: 'positive_color_jitter',
    label: 'positive color jitter',
    type: 'range',
    opts: {
      min: 0,
      max: 360,
  ...
  },
  {
    ...
    type: 'color',
    gen_default: () => 'rgb(213, 140, 56)',
  },
  {
    key: 'negative_color_jitter',
    label: 'negative color jitter',
    type: 'range',
    opts: {
      min: 0,
      max: 360,
  ...
  {
    key: 'invert',
    label: 'invert positive/negative',
    type: 'checkbox',
    gen_default: () => false,
    parser: raw => {
...
function create_ui({ state }) {
  ...
  ui.gui = new guify({
    title: '<span>The Baumgartnerizer (<a style="color: inherit" href="https://github.com/adorsk/baumgartnerizer/" target="_blank">ABOUT</a>) <a style="color: inherit" href="https://forms.gle/sPG8z2z9oY3dgSr86" target="_blank">(Guestbook)</a></span>',
    open: true,
  })
  for (const param_def of PARAM_DEFS) {
...
async function render({ ui, state }) {
  ...
  draw_grayscale_image_to_canvas({
    image,
  ...
  decorate_rows_with_grayscale_values({ rows, canvas: ui.input_canvas })
  draw_rows({
    rows,
...
function draw_grayscale_image_to_canvas({ image, canvas }) {
...
function decorate_rows_with_grayscale_values({ rows, canvas }) {
  const image_data = canvas.getContext('2d').getImageData(0, 0, canvas.width, canvas.height)
  for (const row of rows) {
    for (const cell of row.cells) {
      cell.grayscale_value = compute_grayscale_value_for_region({
        region: {
          type: 'rect',
          x: cell.x,
          y: cell.y,
...
function compute_grayscale_value_for_region({ region, image_data }) {
  const pixels = get_pixels_for_region({ region, image_data })
  const grayscale = compute_grayscale_value_for_pixels({ pixels })
  return grayscale
}
...
function draw_rows({ rows, canvas, params, }) {
  ...
  for (const row of rows) {
    ...
    const y = cell.y + (0.5 * cell.height)
    const path_height = params.base_amplitude * cell.height
    ...
    ctx.fillStyle = jitter_rgb({
      rgb_string: params.negative_color,
      jitter: params.negative_color_jitter,
      random: prng,
    })
    ctx.fillRect(0, cell.y, ctx.canvas.width, cell.height)
    ctx.fillStyle = jitter_rgb({
      rgb_string: params.positive_color,
      jitter: params.positive_color_jitter,
      random: prng,
    })
